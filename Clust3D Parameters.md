...
**imputation**: String. The method of imputing the missing values in the data file. (Default: "zeros").
...
*Advanced*:  
	The user can specify the range for the "minmax" scaling (Default: [0–1]) by specifying the range in `Clust3D/main.py`.

______________________________________________________________________________


**scaling_per_dimension**: Boolean. Controls whether scaling is applied independently per dimension or jointly across all dimensions. (Default: False)

	True: 	Each dimension is scaled independently. A separate scaler is fitted and applied
			to each dimension, preserving the relative structure within it.
			This is the recommended setting when the dimensions of the input tensor
			have different value ranges or distributions.

	False: 	All dimensions are scaled jointly after reshaping the tensor into a 2D matrix.
			A single scaler is fitted across all dimensions combined.

*Tips*  
Setting `scaling_per_dimension=True` is recommended when the dimensions of the input
tensor differ substantially in dynamic range, to avoid one dimension dominating the
distance computations. This parameter has no effect when `scaling="none"`.
...
*Advanced*    
The user can plot the SSE plot by uncommenting the plt block in `Clust3D/auto_neuron_number_selection.py`.
...
By selecting "points", Clust3D randomly selects data points equal to the no. of neurons and calculates their average in-between distance. The combination with the highest average distance is selected as the chosen data points to initialize the neurons. This way, Clust3D initializes the neurons by trying to utilize the largest possible span in the high-dimensional data space. The number of different combinations to be calculated is dictated by the "depth" parameter.

*Tips*  
Due to stochasticity, a good practice is to redo the training a couple of times for whichever neuron initialization technique is used.
...
• Very high values (> 500000) can result in very high running times, depending on the user's hardware and the number of neurons.  
...
*Advanced*  
• Being able to select and calculate the dispersion of every possible combination of data points means the exact same initialization for every run of Clust3D. This eliminates the stochastic nature of neuron initialization and results in better consistency.  
...
**random_state**: Positive integer (<=10000). Determines the random state of the algorithm. Choosing a fixed value eliminates the stochastic nature of the "points" neuron initialization and ensures the same combinations are evaluated every time Clust3D is run. (Default: a random value each time Clust3D is run)

______________________________________________________________________________

**All of Clust3D's parameters can be viewed in `Clust3D/main.py` along with their default values.**
